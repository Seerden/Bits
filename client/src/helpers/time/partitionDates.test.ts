...
describe("isSameYear", () => {
    test("returns true if two dates in same year", () => {
        expect(isSameYear(jan1, oct1)).toBeTruthy();
    });

    test("returns false if two dates not in same year", () => {
        expect(isSameYear(jan1, lastJan1)).toBeFalsy();
    });
});
...
describe("partitionDates", () => {
    const dates = [...Array(5).keys()].map((entry) =>
        dayjs(jan1).add(entry, "year").toDate()
    );
    const sameDateArray = [...Array(5).keys()].map((entry) => jan1.toDate());
    const years = [...Array(5).keys()].map((i) =>
        dayjs(new Date())
            .year(2021 + i)
            .toDate()
    );
