 ...
 */
export function partitionDates(dates: Date[], partitions: Date[], timescale: Timestep) {
    const truncateFn = dateTruncateMap[timescale];
    const partitionedDates = partitions
        .map(partitionLabelDate => {
            const truncatedLabel = truncateFn(dayjs(partitionLabelDate));

            const filtered = dates.filter(date =>
                isSameYear(dayjs(partitionLabelDate), dayjs(date)) && (truncateFn(dayjs(date)) === truncatedLabel)
            );
